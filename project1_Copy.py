...
def perceptron_single_step_update(
    ...
    completed.
    """
    if label*(np.dot(current_theta,feature_vector)+current_theta_0) <=0.0001:
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            updated_theta,updated_theta_0=perceptron_single_step_update(feature_matrix[i],labels[i],theta,theta_0)
            if np.any(updated_theta != theta):
                theta = updated_theta
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    prdictd_labels=np.zeros(feature_matrix.shape[0])
...
def classifier_accuracy(
    ...
    train_prdictd_labels=classify(train_feature_matrix, theta, theta_0)
    val_prdictd_labels=classify(val_feature_matrix, theta, theta_0)
    
    return accuracy(train_prdictd_labels, train_labels), accuracy(val_prdictd_labels, val_labels)
    raise NotImplementedError
#pragma: coderesponse end
